...
$vuelta= $sheet->getCell("L".$row)->getValue();	
$fecha =date("y") . "-" . date("m") . "-" . date("d");
$sql_insert="insert into prog_programacion values ('','$planilla','$carga[0]', '$conductor', '$total_cajas','$carga[1]','$n_clientes','$fecha','$cc', '','','', '0','0','$vuelta','')";

$importar->import($sql_insert);
//cuenta corriente
$sql_insert_cc="insert into cc_history values ('$planilla','$fecha','$cc','$carga[0]','$carga','$conductor','','','','','','','','','','','','','','')";
$importar->import($sql_insert_cc);

$sql_insert_cc_valores="insert into cc_valores values('$planilla','','','','','')";
$importar->import($sql_insert_cc_valores);
}
}
